...
class CryptoSignalVisualizer:
    ...
    def _find_price_cross(self, df, signal_time, target_price):
        ...
        try:
            ...
            for i in range(1, len(full_timeline)):
                ...
                if (prev_price < target_price < curr_price) or (prev_price > target_price > curr_price):
        ...
        try:
            ...
            fig.add_trace(go.Candlestick(
                ...
                hovertext=[
                    ...
                    for x, o, h, l, c in zip(df['open_time'], df['open'], df['high'], df['low'], df['close'])
                ]
            ), row=1, col=1)
    ...
    def save_signal_chart(self, symbol, signal_time, signal_type, price1, price2):
        ...
        try:
            ...
            plot_path = os.path.join(self.plot_dir, f"{symbol}_signal_{signal_time.strftime('%Y-%m-%d_%H-%M-%S')}.html")
            fig.write_html(
                plot_path,
                full_html=True,
