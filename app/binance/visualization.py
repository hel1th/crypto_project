...
class CryptoSignalVisualizer:
    ...
    def _ensure_datetime(
        self, dt: Union[str, datetime.datetime, pd.Timestamp]
    ) -> datetime.datetime:
    ...
    def load_data_for_signal(
        ...
        end_time = (
            ...
            else signal_time + datetime.timedelta(minutes=minutes_after)
        )

        filtered_df = df[
            (df["open_time"] >= start_time) & (df["open_time"] <= end_time)
        ].copy()
        ...
        if filtered_df["open_time"].dt.tz is None:
            filtered_df["open_time"] = filtered_df["open_time"].dt.tz_localize(
                self.timezone
            )
        else:
            filtered_df["open_time"] = filtered_df["open_time"].dt.tz_convert(
                self.timezone
            )
    ...
    def _find_price_cross(
        self, df: pd.DataFrame, signal_time: datetime.datetime, target_price: float
    ) -> tuple:
        ...
        try:
            ...
            for i in range(1, len(df_slice)):
                ...
                if (
                    prev_row["low"] <= target_price <= prev_row["high"]
                    or curr_row["low"] <= target_price <= curr_row["high"]
                    or (prev_row["close"] < target_price < curr_row["open"])
                    or (prev_row["close"] > target_price > curr_row["open"])
                ):
            ...
            if cross_time:
                ...
                if additional_end_time > df["open_time"].max():
                    ...
                    df = self.load_data_for_signal(
                        symbol, signal_time, minutes_after=minutes_after
                    )
    ...
    def save_signal_chart(
        ...
        try:
            ...
            html_str = fig.to_html(
                full_html=True, include_plotlyjs="cdn", default_height="80vh"
            )
