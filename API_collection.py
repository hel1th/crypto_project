...
class CryptoDataVisualizer:
    ...
    def __init__(self, symbols=['BTCUSDT', 'ETHUSDT', 'BNBUSDT'], interval='1d', data_dir='crypto_data'):
    ...
    def fetch_crypto_data(self, symbol, start_date=None):
    ...
    def update_data(self):
        ...
        print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Начато обновление данных...")
        ...
        for symbol in self.symbols:
            ...
            if os.path.exists(filepath):
                ...
                new_data = self.fetch_crypto_data(symbol, last_date)
                
                if new_data is not None and not new_data.empty:
                    combined = pd.concat([df, new_data]).loc[~df.index.duplicated(keep='last')]
                    combined.to_csv(filepath)
                    print(f"Данные для {symbol} обновлены. Добавлено {len(new_data)} записей")
    ...
    def generate_plots(self, df, symbol):
        ...
        plt.savefig(os.path.join(self.plot_dir, f'{symbol}_price.png'), bbox_inches='tight', dpi=100)
        plt.close()
        ...
        plt.grid(True, alpha=0.3)
        plt.savefig(os.path.join(self.plot_dir, f'{symbol}_volume.png'), bbox_inches='tight', dpi=100)
        plt.close()

    def daily_update(self):
        """Ежедневное обновление"""
        self.update_data()
        print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Данные и графики обновлены")
    ...
    def run(self):
        ...
        print("Система мониторинга запущена. Графики будут обновляться ежедневно в 00:00")
